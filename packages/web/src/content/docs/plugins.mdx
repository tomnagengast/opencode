...
If you build a provider integration, register it through the `providers` helper on the plugin context. Supplying metadata plus a loader makes the provider available everywhere (desktop, server, and TUI share the same registry):

```ts title=".opencode/plugin/custom-provider.ts"
import type { Plugin } from "@opencode-ai/plugin"

export const CustomProvider: Plugin = async ({ providers, directory }) => {
  providers.register({
    id: "my-provider",
    info: {
      id: "my-provider",
    ...
    loader: () => ({
      ...
      getModel: async (_sdk, modelID, options) => {
        // Return a LanguageModel implementation
      },
    }),
  })
  return {}
